...
function updateMotivation() {
  if (Math.random() < motivProb) {
    if (motiv < 100) {
      motiv+=multi;
      motivProb += 0.002*motiv;
      motivProb = Math.min(0.8, motivProb);
      if (motivProb ==0.8 ) motivProb = 0.25;
    }
    setTimeout(updateMotivation, 100);
  }
  else setTimeout(updateMotivation, tickspeed);
  if (Math.random < 0.5) motivProb -= motivProb*0.02;
  motiv -= motiv*0.01;
  if (motivProb < 0.1) motivProb = 0.1;
  consumed -= 0.02;
  refreshIndicators();
  if (motiv < 5 && loc != "inside") inside();
...
function refreshIndicators() {
  ...
  ele.innerHTML = "You are " +(consumed<30?"hungry.":
    (consumed<80?"peckish.":"full."));
}
...
function consume() {
  ...
  else if (bought.food <=0) log("Oh, right. Fridge's empty.")
  else if (consumed >= 100) log("You're already full.")
  else log("You can't be bothered to get up and have something to eat.")
}
...
function work() {
  if (motiv>20 && consumed >30) {
    ...
    } else log("After an hour, nothing happens. Guess the coin dispenser broke again.")
    refreshIndicators();
  }
  else if (consumed <= 30) log("You're hungry and don't want to work.")
  else log("You can't be bothered to get up.")
}
...
function outside() {
  ...
  else log("It's cold outside. You don't want to go outside.")
}
